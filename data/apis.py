...
from utils.error_handling import APIKeyMissingError, DataFetchingError

from .models import FundamentalsData, HistoricalData, StockData, create_stock_data
...
class UnifiedDataFetcher:
    def __init__(self, symbol: str):
        ...
        self.cache_file = (
            self.cache_dir / f"{symbol}_{'1y'}.json"
        )  # Assuming 1y period, adjust if needed
    ...
    def _fetch_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"Successfully fetched data from {source_name} for {self.symbol}"
                )
                return data
            except Exception as e:
    ...
    def _fetch_yahooquery_history(self, period: str, interval: str) -> pd.DataFrame:
        ...
        if df.empty or "close" not in df.columns or df["close"].isnull().all():
            raise DataFetchingError(
                "Yahooquery returned empty or invalid dataframe.", "yahooquery"
            )
        ...
        if (df["Close"] == 0).all() or (df["Volume"] == 0).all():
            raise DataFetchingError(
                "All Close or Volume values are zero.", "yahooquery"
            )
...
def _fetch_world_bank_indicator(indicator_code: str) -> Optional[Dict[str, Any]]:
    ...
    def _fetch_yfinance_history(self, period: str, interval: str) -> pd.DataFrame:
        ticker_symbol = self.symbol.replace(
            ".NS", ".NS"
        )  # yfinance handles .NS directly
    ...
    def _fetch_alpha_vantage_history(self) -> pd.DataFrame:
        ...
        if (df["Close"] == 0).all() or (df["Volume"] == 0).all():
            raise DataFetchingError(
                "All Close or Volume values are zero.", "alpha_vantage"
            )
    ...
    def _generate_synthetic_data(self, period: str = "1y") -> pd.DataFrame:
        ...
        try:
            ...
            np.random.seed(
                hash(self.symbol) % 2**32
            )  # Reproducible seed based on symbol
            ...
            for i, date in enumerate(dates):
                ...
                open_price = (
                    prices[i - 1] if i > 0 else price * (1 + np.random.normal(0, 0.005))
                )
    ...
    def get_historical_data(
        ...
        if self.symbol.endswith(".NS"):
            equity_fetchers.extend(
                [
                    (
                        lambda period=period,
                        interval=interval: self._fetch_yahooquery_history(
                            period, interval
                        ),
                        "yahooquery",
                    ),
                    (
                        lambda period=period,
                        interval=interval: self._fetch_yfinance_history(
                            period, interval
                        ),
                        "yfinance",
                    ),
        ...
        for fetch_func, source_name in fetch_sources:
            try:
                ...
                logger.warning(
                    f"Could not fetch data from {source_name} for {self.symbol}: {e}"
                )
                if isinstance(e, DataFetchingError) and getattr(
                    e, "non_retriable", False
                ):
                    break
        ...
        if df is None or df.empty:
            if not _is_cache_disabled() and self.cache_file.exists():
                try:
                    ...
                    if cached_synthetic is None:
                        cached_synthetic = (
                            True  # Treat legacy caches as synthetic by default
                        )
                    ...
                    if current_time - cache_timestamp > cache_validity_hours * 3600:
                        logger.info(
                            f"Cache expired for {self.symbol}, attempting fresh fetch"
                        )
            ...
            if df is None or df.empty:
                if ALLOW_SYNTHETIC_DATA:
                    ...
                    if df is None or df.empty:
                        logger.error(
                            f"Failed to generate synthetic data for {self.symbol}"
                        )
                        return []
                    is_synthetic = True
                else:
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                logger.warning(
                    f"Skipping invalid data row for {self.symbol}: {row}. Error: {e}"
                )

        if not historical_data:
            raise DataFetchingError(
                "No valid historical data points after processing.", "processing"
            )
        ...
        try:
            ...
            raise DataFetchingError(
                f"Data quality validation failed: {str(e)}", "quality_validation"
            ) from e
...
class _EnhancedUnifiedDataFetcher:
    ...
    def get_batch_historical_data(
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_symbol = {
                executor.submit(self.get_historical_data, period, interval): symbol
                for symbol in symbols
            }
            results = {}
            for future in concurrent.futures.as_completed(future_to_symbol):
        ...
        try:
            ...
            for idx, date in enumerate(dates):
                ...
                open_price = (
                    prices[idx - 1]
                    if idx > 0
                    else price * (1 + np.random.normal(0, 0.005))
                )
    ...
    def get_historical_data(
        ...
        for fetch_func, source_name in fetch_sources:
            try:
                ...
                logger.warning(
                    f"Could not fetch data from {source_name} for {self.symbol}: {exc}"
                )
                if isinstance(exc, DataFetchingError) and getattr(
                    exc, "non_retriable", False
                ):
                    break
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                logger.warning(
                    f"Skipping invalid data row for {self.symbol}: {row}. Error: {exc}"
                )
        ...
        try:
            ...
            raise DataFetchingError(
                f"Data quality validation failed: {str(exc)}", "quality_validation"
            ) from exc
...
def _retry_with_backoff(
    func, max_retries: int = 5, base_delay: float = 1.0, source_name: str = "unknown"
):
    last_exception = None
    non_retriable = False
    ...
    failure_exc = DataFetchingError(
        f"Failed to fetch data from {source_name}: {message}", source=source_name
    )
    if non_retriable:
        setattr(failure_exc, "non_retriable", True)
    raise failure_exc from last_exception
...
def get_fundamentals(symbol: str) -> FundamentalsData:
    ...
    if not ALPHA_VANTAGE_API_KEY and not is_mock_fundamental:
        logger.warning(
            "ALPHA_VANTAGE_API_KEY is not configured; skipping fundamental data fetch."
        )
    ...
    def _fetch_alpha_vantage_fundamentals():
        ...
        if not overview and not income_stmt and not balance_sheet and not cash_flow:
            raise DataFetchingError(
                "All fundamental data endpoints returned empty.", "alpha_vantage"
            )

        return {
            "symbol": symbol,
            "overview": overview or {},
            "income_statement": income_stmt.get("annualReports", [])[:1]
            if income_stmt
            else [],
    ...
    def _fetch_yfinance_fundamentals():
        ...
        return {
            "symbol": symbol,
            "overview": overview,
            "income_statement": financials.to_dict("records")[:1]
            if not financials.empty
            else [],
            "balance_sheet": balance.to_dict("records")[:1]
    ...
    for fetch_func, source_name in fetch_sources:
        try:
            ...
            logger.warning(
                f"Could not fetch fundamentals from {source_name} for {symbol}: {e}"
            )
...
def get_stock_info(symbol: str) -> Dict[str, Any]:
    ...
    fetchers = [(_fetch_yahoo_info, "yahooquery"), (_fetch_yfinance_info, "yfinance")]
    ...
    for fetch_func, source_name in fetchers:
        try:
            ...
            last_error = str(e)
            logger.warning(
                f"Failed to fetch stock info for {symbol} via {source_name}: {e}"
            )
...
def get_news_articles(symbol: str, max_articles: int = 10) -> List[Dict[str, Any]]:
    def _fetch_news_api_articles():
        ...
        if all_articles.get("status") != "ok":
            raise DataFetchingError(
                f"NewsAPI error: {all_articles.get('message', 'Unknown')}", "newsapi"
            )
    ...
    def _fetch_fred_data():
        ...
        series_map = {
            "INDUNEMP": (
                "Unemployment Rate",
                "UNRATE",
            ),  # India Unemployment -> US Unemployment
            "INDCPIALLMINMEI": ("Inflation Rate", "CPIAUCSL"),  # India CPI -> US CPI
            "SPASTT01INM661N": (
                "GDP Growth Rate YoY",
                "GDPC1",
            ),  # India GDP Growth -> US GDP
        }
            ...
            try:
                ...
                if "GDP" in indicator_name and "Growth" not in indicator_name:
                    if len(series) >= 5:  # Quarterly data needs ~5 points for YoY
                        ...
                        growth = (
                            ((latest_value - prev_year_value) / prev_year_value) * 100
                ...
                logger.warning(
                    f"Failed to fetch '{indicator_name}' from FRED (Series: {series_id}): {e}. Trying fallback '{fallback_id}'."
                )
                try:
                    series = fred.get_series_latest_release(fallback_id)
                    if series.empty:
                        raise DataFetchingError(
                            f"Fallback series '{fallback_id}' is empty.", "fred"
                        )
                    ...
                    logger.info(
                        f"Fetched fallback {indicator_name} from FRED (Series: {fallback_id})"
                    )
                except Exception as e2:
                    logger.error(
                        f"Failed to fetch fallback for {indicator_name} (Series: {fallback_id}): {e2}"
                    )
...
def get_macro_data() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(
            f"Could not fetch any data from FRED: {e}. Proceeding with defaults."
        )
    ...
    if "GDP Growth Rate YoY" not in macro_data:
        ...
        if gdp_data:
            ...
            macro_data["GDP Growth Rate YoY"] = {
                "value": 6.8,
                "date": "default",
                "source": "default",
            }
