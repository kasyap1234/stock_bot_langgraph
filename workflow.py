...
def log_technical_analysis(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        if failure_count > 3:
            raise RuntimeError(f"Circuit breaker activated: too many analysis failures ({failure_count})")
        logger.exception(f"Technical analysis failed: {e}")
        end_time = datetime.datetime.now()
        logger.info(f"Technical analysis failed at {end_time} (duration: {end_time - start_time})")
        return {"analysis_failure_count": failure_count, "technical_signals": {"error": str(e)}}
...
def log_fundamental_analysis(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        end_time = datetime.datetime.now()
        logger.info(f"Fundamental analysis failed at {end_time} (duration: {end_time - start_time})")
        return {"analysis_failure_count": failure_count, "fundamental_analysis": {"error": str(e)}}
...
def log_sentiment_analysis(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        end_time = datetime.datetime.now()
        logger.info(f"Sentiment analysis failed at {end_time} (duration: {end_time - start_time})")
        return {"analysis_failure_count": failure_count, "sentiment_scores": {"error": str(e)}}
...
def log_macro_analysis(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        end_time = datetime.datetime.now()
        logger.info(f"Macro analysis failed at {end_time} (duration: {end_time - start_time})")
        return {"analysis_failure_count": failure_count, "macro_scores": {"error": str(e)}}
    ...
    try:
        ...
        def log_risk_assessment(state: State) -> Dict[str, Any]:
            ...
            try:
                ...
                end_time = datetime.datetime.now()
                logger.info(f"Risk assessment failed at {end_time} (duration: {end_time - start_time})")
                return {"analysis_failure_count": failure_count, "risk_metrics": {"error": str(e)}}
...
def invoke_workflow(stocks_to_analyze: Optional[list] = None, backtest: bool = False, basic: bool = False, period: str = "5y", walk_forward_enabled: bool = True) -> Optional[State]:
    try:
